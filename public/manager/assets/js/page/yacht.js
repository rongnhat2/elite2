const View = {
    ...
    Service: {
        ...
        setVal(resource, data){
            var json_data = JSON.parse(data)["services"];
            json_data.map(v => {
                $(".services-list")
                    .append(View.Service.render(v))
            })
        },
        getVal(){
            var services = `{ "services": [] }`;
            var servicesList = JSON.parse(services);
            console.log($('.services-list .timeline-item-data').length);
            if ($('.services-list .timeline-item-data').length == 0) {
                return null;
            }else{
                $('.services-list .timeline-item-data').each(function(index, el) {
                    servicesList
                        .services
                        .push(
                            JSON.parse(`{ "name": "${$(this).find(".data-item-name").val().replaceAll( '"', '“' )}", 
        ...
        doCreate(){
            $(document).on('click', `.services-action`, function() {
                $('.services-list').append(View.Service.item);
            }); 
        },
        doRemove(){
            $(document).on('click', `.item-remove`, function() {
        ...
        init(){
            View.Service.doCreate();
            View.Service.doRemove();
        }
    },
    Policy: {
        setVal(resource, data){
            var json_data = JSON.parse(data)["policy"][0];
    ...
    FullTab: {  
        Create: { 
            ...
            getVal(resource){ 
                ...
                var data_regulation  = View.Service.getVal();
                ...
                if (data_regulation == null) { required_data.push('Nhập thông tin quy định.'); onPushData = false }
                ...
                if (onPushData) {
                    ...
                    fd.append('data_regulation', data_regulation);
        ...
        Update: { 
            setVal(resource, data){
                ...
                View.Policy.setVal(resource, data.vehicle);
                View.Timeline.setVal(resource, data);
                View.Utilities.setVal(resource, data);
                View.Service.setVal(resource, data.policy);
            },
            getVal(resource){ 
                var fd = new FormData();
                var required_data = [];
                ...
                // --Required Value
                if (data_name == '') { required_data.push('Nhập tên.'); onPushData = false }
                if (data_prices == '') { required_data.push('Nhập giá.'); onPushData = false }
                if (data_position == null) { required_data.push('Chọn địa điểm.'); onPushData = false }
                if (data_map == '') { required_data.push('Nhập bản đồ.'); onPushData = false }
                if (data_description == '') { required_data.push('Nhập mô tả ngắn.'); onPushData = false }
                if (data_service == '') { required_data.push('Nhập thông tin dịch vụ.'); onPushData = false }
...
(() => {
    ...
    View.FullTab.onPush("Delete", "#popup-delete", () => {
        ...
        View.FullTab.default();
        IndexView.helper.showToastProcessing('Process', 'Đang xử lí');
        Api.Yacht.Delete($("#popup-delete").find('.data-id').val())
            .done(res => { 
                IndexView.helper.showToastSuccess('Success', 'Thành công');
                getData();
            })
    ...
    View.FullTab.onPush("Delete", "#popup-delete-room", () => {
        ...
        View.FullTab.default();
        IndexView.helper.showToastProcessing('Process', 'Đang xử lí');
        Api.YachtRoom.Delete($("#popup-delete-room").find('.data-id').val())
            .done(res => { 
                IndexView.helper.showToastSuccess('Success', 'Thành công');
                getDataRoom($('.data-hotel-id').val());
            })
    ...
    function getDataRoom(id){
        Api.YachtRoom.GetAll(id)
            .done(res => {
                $(".hotel-name").html(res.data.yacht.name)
                IndexView.table.clearRows();
                Object.values(res.data.room).map(v => {
