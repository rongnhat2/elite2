    ...
    table: {
        __generateDTRow(data){ 
            return [
                ...
                data.name,  
                data.type == 1 ? "Trong nước" : "Quốc tế",  
    ...
    FullTab: {  
        ...
        Update: { 
            setVal(resource, data){
                ...
                $(`${resource}`).find(`[name=location][value=${data.type}]`).prop('checked', true);
            },
            getVal(resource){ 
                var fd = new FormData();
                var required_data = [];
...
(() => {
    ...
    View.FullTab.onPush("Delete", "#popup-delete", () => {
        ...
        View.FullTab.default();
        IndexView.helper.showToastProcessing('Process', 'Đang xử lí');
        Api.Location.Delete($("#popup-delete").find('.data-id').val())
            .done(res => { 
                IndexView.helper.showToastSuccess('Success', 'Thành công');
                getData();
            })
