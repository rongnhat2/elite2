                ...
                $(".inland-item")
                    .append(`<div class="tour-wrapper">
                                <div class="I-tour tour-item">
                                    ${data_discount}
                                    <a href="/combo-detail/${v.id}-${v.slug}" class="item-image" style="background-image: url('/${v.images.split(",")[0]}')"></a>
                                    <div class="item-desctiption">
                                        <a href="/combo-detail/${v.id}-${v.slug}" class="item-name">${v.name}</a>
                                        <div class="item-time">
...
(() => { 
    ...
    function getTour(){
        Api.Combo.GetComboNew()
            .done(res => {
                View.Combo.render(res.data)
            })
            .fail(err => { IndexView.helper.showToastError('Error', 'Có lỗi sảy ra'); })
            .always(() => { });
    }
