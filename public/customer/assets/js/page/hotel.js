                ...
                $(".inland-item")
                    .append(`<div class="tour-wrapper">
                                <div class="I-tour tour-item">
                                    ${data_discount}
                                    <a href="/hotel-detail/${v.id}-${v.slug}" class="item-image" style="background-image: url('/${v.images.split(",")[0]}')"></a>
                                    <div class="item-desctiption">
                                        <a href="/hotel-detail/${v.id}-${v.slug}" class="item-name">${v.name}</a>
                                        <div class="item-time">
...
(() => { 
    ...
    function getTour(){
        Api.Hotel.GetHotelNew()
            .done(res => {
                View.Hotel.render(res.data)
            })
            .fail(err => { IndexView.helper.showToastError('Error', 'Có lỗi sảy ra'); })
            .always(() => { });
    }
